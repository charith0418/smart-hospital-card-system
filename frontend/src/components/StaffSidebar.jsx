...
import { LuHospital as HospitalIcon } from "react-icons/lu"; 
...
const StaffSidebar = ({ onLogout }) => {
  ...
  // Removed Logout from main array so we can place it firmly at the bottom
  const navItems = [
    ...
    { name: "QR Generator", icon: <HospitalIcon /> },
  ...
  return (
    <div className="flex bg-gray-100 h-screen w-full overflow-hidden">
      ...
      {/* Sidebar */}
      <div
        className={`fixed bg-[#078a72] w-64 h-screen shadow-lg transition-transform duration-300 z-50 flex flex-col justify-between ${
      ...
      >
          ...
          <div className="p-4 space-y-2">
            {navItems.map((item) => (
              <div
                ...
                className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer select-none transition ${
                  activeTab === item.name
        ...
        {/* Fixed Logout Button at the bottom of the sidebar wrapper */}
        <div className="p-4 border-t border-white/10 mb-2">
          <div
            ...
            className="flex items-center gap-3 p-3 rounded-xl cursor-pointer select-none text-white hover:bg-red-500/20 hover:text-red-200 transition duration-200"
